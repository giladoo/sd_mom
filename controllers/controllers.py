...
class Apps(http.Controller):
    ...
    # @http.route('/sd_mom/attendance/', type='http', auth="public", methods=['GET'], website=True)
    def sd_mom_http(self, **kwargs):
        ...
        if search:
            ...
            apps = http.request.env['sd_mom.settings'].sudo().search(['|',
                                                                       ('name', 'like', list[0]),
                                                                       ('link', 'like', list[0]),
            ...
            apps = http.request.env['sd_mom.settings'].sudo().search([], order='priority,name asc')
            return self.render_http(apps,time_1, today)
    ...
    def render_http(self,apps,time_1,today,search=''):
        ...
        print(Fore.RED,count)
        print(Fore.RED,count)
        return http.request.render('sd_mom.apps', {'apps': apps,
                                                    'count': count,
                                                    'search': search,
                                                    'duration': duration,})
